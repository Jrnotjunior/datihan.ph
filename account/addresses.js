...
const openModal = (address = null) => {
    ...
    if (title) {
        title.textContent = address ? 'Edit address' : 'Add address';
    }
...
const render = () => {
    ...
    addresses.forEach((address) => {
        ...
        card.className =
            `address-card${address.is_default ? ' default' : ''}`;
        ...
        const street = document.createTextNode(
            address.address_line || ''
        );

        const br1 = document.createElement('br');

        const cityProvince = document.createTextNode(
            `${address.city || ''}, ${address.province || ''} ${address.postal_code || ''}`
        );

        const br2 = document.createElement('br');

        const country = document.createTextNode('Philippines');

        lines.appendChild(street);
        lines.appendChild(br1);
        lines.appendChild(cityProvince);
        lines.appendChild(br2);
        lines.appendChild(country);
        ...
        editButton.className =
            'button button-secondary edit-address';
...
const loadAddresses = async (session) => {
    if (!session?.user?.id) {
        throw new Error(
    ...
    console.log(
        '[Addresses] Loading addresses for user:',
        session.user.id
    );
...
const validate = () => {
    ...
    const postal =
        document.getElementById('address-postal')
            ?.value.trim() || '';
...
async function saveAddress(event) {
    ...
    const button =
        document.getElementById('save-address-button');
    ...
    try {
         ...
         * remove the default flag from the user's
         ...
         * This ensures the UI shows the actual
         * database record, not just local data.
         */

        await loadAddresses(authSession);
...
async function setDefaultAddress(id) {
    try {
        ...
        console.error(
            '[Addresses] Default address error:',
            error
        );
...
const setupEvents = () => {
    ...
    document
        .getElementById('address-modal-backdrop')
        ?.addEventListener(
            'click',
            closeModal
        );
...
const getCurrentSession = async () => {
    ...
    /*
     * IMPORTANT:
     * Do not rely only on datihan-auth-ready.
     *
     * The previous version could miss that event,
     * leaving the page permanently on
     * "Loading addresses...".
     */
...
const initAddressPage = async () => {
    ...
    try {
        ...
        await loadAddresses(session);

    } catch (error) {
        console.error(
            '[Addresses] Initialization error:',
            error
        );
