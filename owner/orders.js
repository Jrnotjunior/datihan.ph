document.addEventListener('DOMContentLoaded', () => {
  ...
  const state = { orders: [], productIds: new Set() };
  ...
  function getShipping(order) {
    ...
    return {
      ...
      line: [address.address_line, address.city, address.province, address.postal_code].filter(Boolean).join(', ') || 'No shipping address'
    };
  }
  ...
  function render() {
    ...
    const visible = state.orders.filter(order => {
      ...
      const matchesStatus = selectedStatus === 'all' || String(order.status || 'pending') === selectedStatus;
      const haystack = `${order.order_number || ''} ${shipping.name} ${shipping.phone}`.toLowerCase();
      return matchesStatus && (!query || haystack.includes(query));
    });

    if (!visible.length) {
      list.innerHTML = '<div class="empty-state"><h2>No orders found.</h2><p>Orders matching your shop will appear here when customers place them.</p></div>';
      return;
    }
  ...
  function openOrder(order) {
    ...
    detailContent.innerHTML = `
      <div class="modal-header">
        <div><p class="eyebrow">ORDER DETAILS</p><h2>${escapeHtml(order.order_number || `Order ${order.id}`)}</h2></div>
      ...
      <div class="detail-section"><h3>Items</h3>${items.map(item => `
        <div class="detail-item"><div><strong>${escapeHtml(item.product_name || 'Product')}</strong><span>Qty ${Number(item.quantity || 1)}</span></div><strong>${money(item.subtotal ?? Number(item.price || 0) * Number(item.quantity || 1))}</strong></div>
      `).join('')}</div>
  ...
  async function loadOrders() {
    ...
    try {
      ...
      const productIds = (productsResult.data || []).map(product => String(product.id));
      state.productIds = new Set(productIds);

      if (!productIds.length) {
        state.orders = [];
        list.innerHTML = '<div class="empty-state"><h2>No products assigned to your shop yet.</h2><p>Add products with your owner account first. Orders containing your products will appear here.</p></div>';
        return;
      }
      ...
      list.innerHTML = '<div class="empty-state"><h2>We could not load your orders.</h2><p>Please check the Supabase products/order policies and refresh the page.</p></div>';
      showToast(error.message || 'Unable to load orders.', true);
    }
  }
